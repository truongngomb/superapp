...
import { Plus, Shield, Search, RefreshCw, Loader2, Trash2, FileSpreadsheet } from 'lucide-react';
...
export default function RolesPage() {
  ...
  }, [debouncedSearch, showArchived, sortConfig.field, sortConfig.order, fetchRoles]);
  ...
  const hasDeletedSelected = selectedIds.some(id => 
    roles.find(r => r.id === id)?.isDeleted
  );
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-start gap-3">
          ...
          <PermissionGuard resource="roles" action="view">
            <button
               ...
               aria-label={t("common:export_excel", { defaultValue: "Export Excel" })}
            >
              {exporting ? (
        ...
        <PermissionGuard resource="roles" action="create">
          <Button onClick={() => { setEditingRole(undefined); setShowFormModal(true); }}>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <SortPopup
          ...
          onSort={handleSort}
        />
          <Button variant="outline" onClick={() => { void fetchRoles({ page: 1, sort: sortConfig.field, order: sortConfig.order ?? undefined, search: searchTerm, isDeleted: showArchived }); }}>
        ...
        <Card className="bg-muted/50 border-dashed">
          <CardContent className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-1">
              {searchTerm ? t('roles:list.empty_search') : t('roles:list.empty')}
            </h3>
            <p className="text-muted-foreground mb-4 max-w-sm">
              {searchTerm
                ? t('common:try_adjusting_search', { defaultValue: 'Try adjusting your search terms' })
                : showArchived
            ...
            <div className="grid gap-4">
              {roles.map((role, index) => (
                <RoleRow
                  ...
                  data={{
                    roles,
                    onEdit: (r) => { setEditingRole(r); setShowFormModal(true); },
          ...
          {pagination.totalPages > 1 && (
            <div className="my-4 relative">
              ...
              <Pagination
                ...
                onPageChange={(page) => fetchRoles({ page, search: debouncedSearch, sort: sortConfig.field, order: sortConfig.order ?? undefined, isDeleted: showArchived })}
              />
            </div>
          )}
        </div>
      ...
      <ConfirmModal
        ...
        message={t('roles:restore_confirm', { defaultValue: 'Are you sure you want to restore this role?' })}
      ...
      <ConfirmModal
        ...
        title={t('common:batch_delete_title', { defaultValue: 'Delete Selected Items' })}
        message={
          hasDeletedSelected
           ? t('roles:batch_hard_delete_confirm', { count: selectedIds.length, defaultValue: `Are you sure you want to permanently delete ${selectedIds.length} items?` })
      ...
      <ConfirmModal
        ...
        title={t('common:batch_restore_title', { defaultValue: 'Restore Selected Items' })}
        message={t('roles:batch_restore_confirm', { count: selectedIds.length })}
      ...
      <ConfirmModal
        ...
        message={t("roles:batch_status_confirm", {
          ...
          defaultValue: `Are you sure you want to ${batchStatusConfig?.isActive ? 'activate' : 'deactivate'} ${selectedIds.length} roles?`
        })}
