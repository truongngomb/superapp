...
export function useRoles() {
  ...
  const deleteRoles = async (ids: string[]) => {
    ...
    try {
      ...
      const successMessage = hasDeleted
        ? t('toast.batch_hard_delete_success' as any, { count: ids.length, defaultValue: 'Successfully permanently deleted {{count}} roles' })
        : t('toast.batch_delete_success' as any, { count: ids.length, defaultValue: 'Successfully deleted {{count}} roles' });
  ...
  const updateRolesStatus = async (ids: string[], isActive: boolean) => {
    ...
    try {
      ...
      toast.success(t('toast.batch_status_success' as any, { count: ids.length, defaultValue: 'Successfully updated status for {{count}} roles' }));
      await reloadRoles();
      return true;
    } catch (error) {
      const message = error instanceof ApiException ? error.message : t('toast.error' as any);
  ...
  const restoreRole = async (id: string) => {
    ...
    try {
      ...
      toast.success(t('toast.restore_success' as any, { defaultValue: 'Role restored successfully' }));
  ...
  const restoreRoles = async (ids: string[]) => {
    ...
    try {
      ...
      toast.success(t('toast.batch_restore_success' as any, { count: ids.length, defaultValue: 'Successfully restored {{count}} roles' }));
      await reloadRoles();
      return true;
    } catch (error) {
      const message = error instanceof ApiException ? error.message : t('toast.error' as any);
