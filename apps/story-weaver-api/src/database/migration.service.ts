    ...
    for (const collectionDiff of diff.collections) {
      ...
      try {
        if (collectionDiff.action === 'create') {
          ...
          // Update map with new collection ID so subsequent collections 
          // can resolve relations to this one
          try {
            const newCol = await this.pb.collections.getOne(schema.name);
            this.collectionIdMap.set(newCol.name, newCol.id);
            this.collectionIdToNameMap.set(newCol.id, newCol.name);
          } catch {
            log.warn(`Could not fetch ID for newly created collection ${collectionDiff.name}`);
          }
