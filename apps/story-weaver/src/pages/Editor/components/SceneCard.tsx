...
import { Card, Button, MediaLightbox } from '@superapp/ui-kit';
import { GripVertical, Image as ImageIcon, Music as MusicIcon, Clock, Trash2, Maximize2 } from 'lucide-react';
    ...
    return (
        <>
            <Card className="p-4 mb-4 flex gap-4 items-start group relative">
                ...
                <div 
                    className="w-1/4 aspect-video bg-muted rounded-md flex items-center justify-center overflow-hidden border border-border group/image relative cursor-pointer"
                    onClick={() => { if (scene.imageUrl) setViewerOpen(true); }}
                >
                    {scene.imageUrl ? (
                        <>
                            ...
                            <div className="absolute inset-0 bg-black/0 group-hover/image:bg-black/20 transition-colors flex items-center justify-center">
                                <Maximize2 className="text-white w-6 h-6 opacity-0 group-hover/image:opacity-100 transition-opacity drop-shadow-md" />
                            </div>
                        </>
                    ) : (
            ...
            {scene.imageUrl && (
                <MediaLightbox
                    ...
                    items={[{ 
                        id: scene.id, 
                        url: scene.imageUrl, 
                        title: `${t('video_projects:editor.scene_label', { index: index + 1 })} - ${(scene.scriptText || '').slice(0, 50)}...`  
                    }]}
                />
            )}
        </>
