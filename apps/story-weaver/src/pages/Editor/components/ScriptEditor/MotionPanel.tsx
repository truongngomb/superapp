...
export const MotionPanel = ({
  ...
  const handleAnimate = () => {
    generateMotion(
      ...
      {
        onSuccess: () => {
          toast.success(t('video_projects:motion.generate_success', { defaultValue: 'Motion clip generated!' }));
        },
        onError: (error) => {
          const msg = error instanceof Error ? error.message : t('uikit:error.unknown');
          toast.error(`${t('video_projects:motion.generate_error', { defaultValue: 'Failed to animate' })}: ${msg}`);
        }
      }
    );
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-medium flex items-center gap-2">
            ...
            {t('video_projects:motion.title', { defaultValue: 'Scene Motion' })}
          </h4>
          <p className="text-xs text-muted-foreground mt-1">
            {t('video_projects:motion.description', { defaultValue: 'Bring this scene to life with AI motion' })}
          </p>
        </div>
        {hasImage && (
          <Button 
          ...
          >
            ...
            {hasVideo 
              ? t('video_projects:motion.regenerate', { defaultValue: 'Re-animate' }) 
              : t('video_projects:motion.generate', { defaultValue: 'Animate Scene' })
            }
          </Button>
        )}
      </div>
      ...
      {!hasImage ? (
        <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-lg bg-muted/30">
          ...
          <p className="text-sm text-muted-foreground text-center max-w-[250px]">
            {t('video_projects:motion.no_image_hint', { defaultValue: 'Please select or generate a visual keyframe first in the Visuals tab.' })}
          </p>
        </div>
      ) : isGenerating ? (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-center py-4">
            <p className="text-sm text-muted-foreground animate-pulse">
              {t('video_projects:motion.generating_hint', { defaultValue: 'Generating 4s motion clip...' })}
            </p>
          </div>
        </div>
      ) : hasVideo ? (
          ...
          <div className="p-3 rounded-md bg-primary/5 border border-primary/10 flex items-start gap-3">
            ...
            <div className="text-xs text-muted-foreground">
              <p className="font-medium text-primary">
                {t('video_projects:motion.ready_title', { defaultValue: 'Motion Clip Ready' })}
              </p>
              <p className="mt-0.5">
                {t('video_projects:motion.ready_desc', { defaultValue: 'This clip will be used during final video rendering.' })}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="relative aspect-video rounded-md overflow-hidden border border-border bg-muted/20">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-sm font-medium">
                {t('video_projects:motion.ready_to_animate', { defaultValue: 'Ready to Animate' })}
              </p>
              <p className="text-xs text-muted-foreground mt-1 max-w-[200px]">
                {t('video_projects:motion.cta_hint', { defaultValue: 'Click the button above to generate motion.' })}
              </p>
            </div>
          </div>
        </div>
