...
export function ThemeProvider({ children, defaultTheme = THEME.LIGHT }: ThemeProviderProps) {
  ...
  useEffect(() => {
    const dbTheme = user?.preferences?.theme as string | undefined;
    if (!dbTheme) return;
    
    // Check if dbTheme matches allowed values
    if (dbTheme === THEME.LIGHT || dbTheme === THEME.DARK) {
      const validTheme = dbTheme as Theme;
